...
public class ExtensionProxyFactory {
	 ...
	 * @return
	 */
	public static String getStableSimpleName(Class<?> delegateClass, List<Class<?>> contracts) {
		StringBuilder key = new StringBuilder(delegateClass.getName());
		for (Class<?> contract : contracts) {
			key.append('|').append(contract.getName());
		}
		try {
			byte[] digest = java.security.MessageDigest.getInstance("SHA-256")
					.digest(key.toString().getBytes(java.nio.charset.StandardCharsets.UTF_8));
			StringBuilder hex = new StringBuilder("Extension_");
			for (int i = 0; i < 12; i++) {
