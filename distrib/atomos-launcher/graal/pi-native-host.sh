#!/bin/bash
# On the Pi host (no container): run the native binary with and without the CRL cache, Kura service stopped meanwhile.
set -u
B=/root/kura-bench; W=$B/graal-work; EX=$(cat $B/graal/profile-min.exclude); LIB=$B/host_lib/atomos_lib
rm -rf $B/host_lib; mkdir -p $LIB
...
ln -s $B/atomos/patches/2s/osgi-resource-locator-1.0.3.jar $LIB/2s__osgi-resource-locator-1.0.3.jar
ln -s $B/atomos/lib/org.apache.felix.atomos-1.0.0.jar $LIB/; ln -s /opt/eclipse/kura/plugins/org.eclipse.osgi-3.21.0.jar $LIB/
echo "lib jars: $(ls $LIB | wc -l)"
D=/opt/eclipse/kura
...
run() { # label
  sync; echo 3 > /proc/sys/vm/drop_caches; sleep 2; : > /var/log/kura.log
  ...
  t=$(awk '{print $1}' /proc/uptime); tf=$(awk -v a=$t0 -v b=$t 'BEGIN{printf "%.2f", b-a}')
  sleep 30; echo "$1: first has started ${tf}s, RSS30 $(awk '/VmRSS/{print $2}' /proc/$P/status) kB, CPU $(awk -v tck=100 '{printf "%.1f", ($14+$15)/tck}' /proc/$P/stat) s, log lines $(wc -l < /var/log/kura.log), errors $(grep -c ERROR /var/log/kura.log)"
  kill -TERM $P; wait $P 2>/dev/null; sleep 2
}
CRL=/opt/eclipse/kura/user/security/cacerts.ks.crl
run "native host, CRL cache present ($(stat -c %s $CRL 2>/dev/null) bytes)"
mv $CRL $CRL.bak 2>/dev/null; run "native host, no CRL cache"; mv $CRL.bak $CRL 2>/dev/null; [ -f $CRL ] || echo "WARN: CRL file missing after restore"
run "native host, CRL cache present again"
head -3 $B/host-native.out | cut -c1-120
