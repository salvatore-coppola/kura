#!/bin/bash
# On the Pi: build the REST profile native image, then compare JVM vs native on the host (Kura service stopped meanwhile).
set -u
B=/root/kura-bench; W=$B/graal-work-rest; mkdir -p $W/out
...
RUN="docker run --rm --privileged -e PROFILE=rest -v /opt/eclipse/kura:/opt/eclipse/kura:ro -v $B/atomos:/atomos:ro -v $B/graal:/graal:ro -v $W:/work kura-graal:21"
echo "### $(date -Is) native-image build (rest)"; $RUN bash /graal/build-native.sh -J-Xmx5g > $W/build.log 2>&1; grep -E "Finished generating|Error|Caused by|in total|Peak RSS" $W/build.log | head -4; grep -q "Finished generating" $W/build.log || { echo "build failed, aborting"; exit 1; }
EX=$(cat $B/graal/profile-rest.exclude); PATCHED=$(ls $B/atomos/patches/*/*.jar | xargs -n1 basename | tr '\n' '|' | sed 's/|$//')
LIB=$B/host_lib_rest/atomos_lib; rm -rf $B/host_lib_rest; mkdir -p $LIB; CP=$B/atomos/org.eclipse.kura.atomos.launcher-6.0.0-SNAPSHOT.jar:$B/atomos/lib/org.apache.felix.atomos-1.0.0.jar:/opt/eclipse/kura/plugins/org.eclipse.osgi-3.21.0.jar
...
measure() { # label cmd...
  ...
  sleep 5; local rss1=$(awk '/VmRSS/{print $2}' /proc/$P/status); [ "$label" = "${label#*bench}" ] && bash $B/graal/rest-session-test.sh | sed 's/^/    /' | head -14
...
measure "jvm-rest (C1, Xmx1024)" java $JVMF -XX:TieredStopAtLevel=1 -XX:+UseSerialGC -Xms64m -Xmx1024m $PROPS -cp "$CP" org.eclipse.kura.atomos.KuraAtomosLauncher
measure "native-rest (Xmx512)" $W/out/kura-native -Xmx512m -Datomos.lib.dir=$B/host_lib_rest $PROPS
for n in 2 3; do measure "jvm-rest bench $n" java $JVMF -XX:TieredStopAtLevel=1 -XX:+UseSerialGC -Xms64m -Xmx1024m $PROPS -cp "$CP" org.eclipse.kura.atomos.KuraAtomosLauncher; measure "native-rest bench $n" $W/out/kura-native -Xmx512m -Datomos.lib.dir=$B/host_lib_rest $PROPS; done
echo "### $(date -Is) restore"; chown -R kurad:kurad /opt/eclipse/kura; systemctl start kura; sleep 3; systemctl is-active kura
echo "### $(date -Is) pi native rest (crl fix) done"
