...
sleep 40; echo "RSS kB: $(awk "/VmRSS/{print \$2}" /proc/$P/status)"; grep -E "bundles by state" /work/native-run.out | tail -1; grep -E "state=16" /work/native-run.out | head -3 | cut -c1-160; cp /var/log/kura.log /work/native-kura-rest.log; echo "-- errors: $(grep -cE "ERROR" /var/log/kura.log) log / $(grep -ciE "exception" /work/native-run.out) out"; grep -n -m1 -A25 "reload failed" /var/log/kura.log | grep -E "reload failed|Caused by|Exception|Error" | head -8 | cut -c25-260; kill -TERM $P; wait $P 2>/dev/null; echo "binary: $(stat -c %s /work/out/kura-native)"'
echo "### done $(date -Is)"
