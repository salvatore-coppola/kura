...
set -u
. /graal/common.sh; [ -d /work/native-libs ] && NATIVE_LIBS_OPT="-Djava.library.path=/work/native-libs" || NATIVE_LIBS_OPT=""
CP=$(build_cp); T=${1:-90}
...
java -agentlib:native-image-agent=config-output-dir=/work/cfg,experimental-class-define-support --add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.util=ALL-UNNAMED \
  $KURA_PROPS ${NATIVE_LIBS_OPT:-} ${DIAG_OPTS:-} -Dosgitech.rest.extension.proxy.disabled=true -Dkura.atomos.dump=true -Dkura.atomos.diag=true -Dkura.atomos.dumpAfter=$((T-20)) -cp "$CP" org.eclipse.kura.atomos.KuraAtomosLauncher > /work/agent-run.out 2>&1 &
P=$!; sleep $((T/2))
if [ "$PROFILE" = rest ] || [ "$PROFILE" = jni ]; then
  ...
  [ -n "${MQTT_BROKER:-}" ] && bash /graal/mqtt-test.sh "$MQTT_BROKER"
  [ -n "${MQTT_TLS_BROKER:-}" ] && bash /graal/mqtt-test.sh "$MQTT_TLS_BROKER" "${MQTT_CA:-}"
fi
sleep $((T/2))
grep -E "comm diag|hid diag" /work/agent-run.out; echo "-- states:"; grep -E "bundles by state" /work/agent-run.out | tail -1; grep -E "state=16|^INSTALLED" /work/agent-run.out | grep -vE " (java|jdk)\." | head -5
