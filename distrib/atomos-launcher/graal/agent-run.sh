...
java -agentlib:native-image-agent=config-output-dir=/work/cfg,experimental-class-define-support --add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.util=ALL-UNNAMED \
  $KURA_PROPS ${NATIVE_LIBS_OPT:-} -Dkura.atomos.dump=true -Dkura.atomos.diag=true -Dkura.atomos.dumpAfter=$((T-20)) -cp "$CP" org.eclipse.kura.atomos.KuraAtomosLauncher > /work/agent-run.out 2>&1 &
P=$!; sleep $((T/2))
